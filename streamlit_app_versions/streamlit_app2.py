...
if user_id:
    if check_user_id(int(user_id)):
        ...
        connection = psycopg2.connect(database="postgres", user="postgres", password="postgres", host="flatbot-database.cel1s9qmhs81.eu-central-1.rds.amazonaws.com", port="5432")
        cursor = connection.cursor()
        cursor.execute("SELECT max_price, min_size, min_rooms, max_age, selected_cities, selected_districts, selected_neighborhoods FROM users WHERE user_id = %s", (user_id,))
        ...
        if user_settings is not None:
            user_settings = {
                ...
                "selected_districts":  user_settings[5],
                "selected_neighborhoods": user_settings[6]
            }
            print(user_settings)
        else:
            user_settings = {
        ...
        selected_districts = user_settings["selected_districts"]
        districts = ['Mitte', 'Friedrichshain_Kreuzberg', 'Panow', 'Charlottenburg-Wilmersdorf', 'Spandau', 'Steglitz-Zehlendorf', 'Tempelhof-Schöneberg', 'Neukölln', 'Treptow-Köpenick', 'Marzahn-Hellersdorf', 'Lichtenberg', 'Reinickendorf']
    ...
    'Borsigwalde'
        ...
        cursor.execute("UPDATE users SET max_price = %s, min_size = %s, min_rooms = %s, selected_cities = %s, selected_neighborhoods = %s, selected_districts = %s WHERE user_id = %s",
                       (user_settings["max_price"], user_settings["min_size"], user_settings["min_rooms"], user_settings["selected_cities"], user_settings["selected_neighborhoods"], user_settings["selected_districts"], user_id))
        connection.commit()
        connection.close()
